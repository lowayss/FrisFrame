...
const macInstaller = fs.readFileSync(path.join(root, "electron/scripts/install-mac.cjs"), "utf8");
const packageSmoke = fs.readFileSync(path.join(root, "electron/scripts/smoke-package.cjs"), "utf8");
...
assert.equal(packageJson.scripts["desktop:install:mac"], "node electron/scripts/install-mac.cjs");
assert.equal(packageJson.scripts["desktop:update:mac"],
  "npm run desktop:dir:mac && npm run desktop:verify && npm run desktop:install:mac");
assert.equal(packageJson.scripts["desktop:build:win"], "npm run desktop:prepare && electron-builder --win nsis --x64");
assert.match(macInstaller, /\/Applications/);
assert.match(macInstaller, /\.Trash/);
assert.match(macInstaller, /previous-/);
assert.match(macInstaller, /installedCandidates\(\)/);
assert.match(macInstaller, /\/usr\/bin\/ditto/);
assert.match(macInstaller, /\/usr\/bin\/osascript/);
assert.match(macInstaller, /mkdirSync\(trashDir/);
assert.match(macInstaller, /remaining\.length !== 1/);
assert.match(packageSmoke, /frisframe-package-smoke-/);
assert.match(packageSmoke, /--user-data-dir=/);
assert.match(packageSmoke, /referenceGhostPanel/);
assert.match(packageSmoke, /referenceGhostObservationLayer/);
