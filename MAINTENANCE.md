...
새 계산 규칙은 가능한 한 작은 코어 모듈에 넣고 `app.js`는 화면 연결만 담당하게 합니다. 기능을 제거할 때는 HTML, 이벤트 연결, 저장 스키마, 가이드와 테스트를 함께 검색합니다. 큰 `app.js/index.html` 레거시 블록을 삭제할 때는 이벤트 바인딩과 DOM 계약을 먼저 분리한 뒤 작은 단계로 제거합니다.
