...
| `prompt-block-core.js` | 배우별 시간 코드 Prompt Block 정규화·추가·이동·길이 조절·삭제 |
| `prompt-motion-core.js` | 자연어 모션 요청을 속도·방향·상승·회전 수치로 해석하고, 명시된 포즈만 기존 포즈 코어로 보간 |
...
4. 배우별 Prompt Block 추가, 모션·명시 포즈 문장 편집, 이동·리사이즈·삭제와 자동 저장 확인
5. 2D 확대·이동과 3D 오빗·화면 이동·줌
6. 연속성 경고·예외 처리·확정 차단과 A/B 저장·비교·복원
7. 카메라 프레임, 현재 프레임, 촬영 자료 ZIP과 MP4 프리뷰
8. 모든 내보내기가 프리뷰 뒤 직접 저장되며 자동 다운로드가 없는지 확인
