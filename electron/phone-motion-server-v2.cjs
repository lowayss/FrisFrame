...
function patchMotionHtml(source) {
  ...
  const newSend = 'async function postSnapshot(outgoingCommand=""){seq++;const started=performance.now();const payload={sessionId,seq,sentAt:Date.now(),clientRttMs:lastRttMs,command:outgoingCommand,motion:{...state,calibrationId}};try{const res=await fetch("/input?token="+encodeURIComponent(TOKEN),{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(payload),cache:"no-store",keepalive:true});if(!res.ok)throw new Error(String(res.status));if(state.enabled)setStatus(state.spatial.mode==="webxr"?"6DoF XR":"MOTION ON","ok")}catch{setStatus("연결 끊김","warn")}finally{lastRttMs=clamp(performance.now()-started,0,2000)}}async function send(){if(sendInFlight){sendQueued=true;return}sendInFlight=true;sendQueued=false;const outgoingCommand=command;command="";try{await postSnapshot(outgoingCommand)}finally{sendInFlight=false;if(sendQueued||command)queueMicrotask(send)}}function sendUrgent(outgoingCommand=""){postSnapshot(outgoingCommand)}';
  html = replaceRequired(html, oldSend, newSend, "single-flight-send");
  html = replaceRequired(html, '$("center").addEventListener("click",recenter);', '$("center").addEventListener("click",()=>{recenter();sendUrgent()});', "recenter-send");
  html = replaceRequired(html, '$("rec").addEventListener("click",()=>{command="toggle-record"});', '$("rec").addEventListener("click",()=>{sendUrgent("toggle-record")});', "record-send");
  html = replaceRequired(html, '$("stop").addEventListener("click",()=>{command="stop"});', '$("stop").addEventListener("click",()=>{sendUrgent("stop")});', "stop-send");
