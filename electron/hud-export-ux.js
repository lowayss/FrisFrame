(() => {
  ...
  style.textContent = `
    ...
    .frisframe-3d-help > summary {
      min-height: 29px; display: inline-flex; align-items: center; gap: 5px; padding: 0 8px;
      border: 1px solid rgba(255,255,255,.09); border-radius: 7px; color: #a8b0b9;
      background: rgba(12,16,20,.64); cursor: pointer; list-style: none; font-size: 10px;
      font-weight: 800; user-select: none; backdrop-filter: blur(8px);
    }
    ...
    .frisframe-3d-help > summary::before {
      ...
      border: 1px solid rgba(255,255,255,.14); border-radius: 999px; color: #858e98; font-size: 9px;
    }
    .frisframe-3d-help[open] > summary { color: #d7dde5; border-color: rgba(255,255,255,.18); background: rgba(28,34,40,.94); }
    .frisframe-3d-help .three-shortcuts {
      position: absolute; right: 0; top: calc(100% + 6px); z-index: 42; min-width: 310px;
      max-width: min(520px, 58vw); display: flex !important; flex-wrap: wrap; gap: 5px 9px;
      padding: 9px 10px; border: 1px solid rgba(255,255,255,.11); border-radius: 8px;
      background: rgba(16,20,24,.96); box-shadow: 0 10px 28px rgba(0,0,0,.28); backdrop-filter: blur(10px);
    }
    ...
    .three-wrap:hover .three-jog-container, .three-jog-container:focus-within { opacity: 1; }

    #exportMenu .frisframe-export-range-details { margin: 1px 0 4px; border: 0; }
    #exportMenu .frisframe-export-range-details > summary {
      min-height: 30px; display: flex; align-items: center; gap: 7px; padding: 0 9px;
      border: 1px solid rgba(255,255,255,.08); border-radius: 7px; color: #aeb5be;
      background: rgba(255,255,255,.025); cursor: pointer; list-style: none; font-size: 10px;
      font-weight: 800; user-select: none;
    }
    #exportMenu .frisframe-export-range-details > summary::-webkit-details-marker { display: none; }
    #exportMenu .frisframe-export-range-details > summary::before { content: "›"; font-size: 15px; line-height: 1; transition: transform .12s ease; }
    ...
    #exportMenu .frisframe-export-range-details > summary small {
      margin-left: auto; color: #727a84; font-size: 9px; font-weight: 700; font-variant-numeric: tabular-nums;
    }
    #exportMenu .frisframe-export-range-details #exportRangeTools {
      margin-top: 6px; padding: 8px !important; border-radius: 7px; background: rgba(0,0,0,.08);
    }
    ...
    .frisframe-export-workflow-note { margin: 3px 2px 1px; color: #727983; font-size: 9px; line-height: 1.4; }

    .frisframe-key-nav {
      display: inline-flex; align-items: center; gap: 3px; margin-left: 2px; padding-left: 5px;
      border-left: 1px solid rgba(255,255,255,.07);
    }
    .frisframe-key-nav button {
      min-width: 30px !important; height: 28px; min-height: 28px !important; padding: 0 7px !important;
      color: #aeb5be; font-size: 10px !important; font-weight: 800;
    }
    .frisframe-key-nav kbd { margin-left: 4px; color: #6f7780; font-size: 8px; font-weight: 700; }
    .frisframe-time-stepper {
      display: grid; grid-template-columns: 26px minmax(0, 1fr) 26px; gap: 3px; align-items: center; width: 100%;
    }
    .frisframe-time-stepper > button {
      width: 26px; min-width: 26px !important; height: 28px; min-height: 28px !important; padding: 0 !important;
      border: 1px solid rgba(255,255,255,.08); border-radius: 6px; background: rgba(255,255,255,.025);
      color: #8f98a2; font-size: 13px; font-weight: 800; cursor: pointer;
    }
    .frisframe-time-stepper > button:hover, .frisframe-time-stepper > button:focus-visible {
      color: #d8dee6; border-color: rgba(255,255,255,.18); background: rgba(255,255,255,.06);
    }
    .frisframe-time-stepper #keyTimeInput { width: 100%; min-width: 0; }

    @media (prefers-reduced-motion: reduce) {
      .frisframe-export-range-details > summary::before, .three-wrap .three-jog-container { transition: none !important; }
    }
  `;
  ...
  if (exportMenu && exportPopover && exportRangeTools && !exportMenu.querySelector(".frisframe-export-range-details")) {
    ...
    updateSummary();
    if (exportRangeSummary) new MutationObserver(updateSummary).observe(exportRangeSummary, { childList: true, characterData: true, subtree: true });
  ...
  const jumpAdjacentKey = (direction) => {
    const current = Number.isFinite(Number(keyTimeInput?.value)) ? Number(keyTimeInput.value) : 0;
    const markers = markersForActiveSource().map((marker) => ({ marker, time: markerTime(marker) }))
      .filter((entry) => entry.time !== null).sort((a, b) => a.time - b.time);
  ...
  document.addEventListener("keydown", (event) => {
    ...
    if (event.key === "[") { event.preventDefault(); jumpAdjacentKey(-1); return; }
    if (event.key === "]") { event.preventDefault(); jumpAdjacentKey(1); return; }
